...
void main() {
  testWidgets(
    ...
    await harness.saveWithBleeding(tester, 'leicht');
    ...
    final marks = await harness.db!.marksDao.marksForDay(_day);
  ...
  testWidgets('dismissing the prompt places no mark', (tester) async {
    ...
    await harness.saveWithBleeding(tester, 'stark');
    ...
    expect(await harness.storedMarkTypes(_day), isEmpty,
  ...
  testWidgets('a spotting day (level 1) shows no prompt', (tester) async {
    ...
    await harness.saveWithBleeding(tester, 'Schmierblutung');
