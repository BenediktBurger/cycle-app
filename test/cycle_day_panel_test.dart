...
void main() {
  ...
  testWidgets(
      ...
      'over the options', (tester) async {
    final (db, _) = await pumpCycleList(tester,
        entries: scenarioEntries,
        seedMarks: [scenarioPeakMark, scenarioFirstHigherMark]);
    ...
    expect(find.text('Low measurement 4'), findsNothing,
    ...
    expect(
        await storedMarkTypes(db, scenarioDay(14)), ['firstHigherMeasurement'],
        reason: 'B\'s own seeded mark is untouched');
    expect(dotPainterOrNull(tester, 8), isA<RingDotPainter>(),
        reason: 'the chart overlay is unchanged by the retarget (9/14 is '
  ...
  testWidgets('the close button clears the panel', (tester) async {
    ...
    expect(find.byKey(const ValueKey('cycleDayPanel')), findsNothing,
  ...
  testWidgets(
      ...
      '(the "tap another day without deselecting" scenario)', (tester) async {
    useTallSurface(tester);
    ...
    await tester.pumpAndSettle();
    expect(await storedMarkTypes(db, scenarioDay(12)), ['mucusPeakDay'],
        reason: 'the first day\'s mark survives — the panel retarget, not a '
            'modal dismissal, moves the options');
    expect(
        await storedMarkTypes(db, scenarioDay(13)), ['firstHigherMeasurement'],
