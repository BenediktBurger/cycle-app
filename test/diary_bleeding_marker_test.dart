...
Finder _tileMarker(DateTime day) => find.descendant(
      of: find.ancestor(
          of: find.text(_tileTitle(day)), matching: find.byType(ListTile)),
...
void main() {
  testWidgets(
    ...
    await tester.pumpWidget(_markerHarness.scope(seed: (db) async {
      ...
      await db.entriesDao.upsertDaily(DailyEntry(
          date: _spottingDay, bbtC: 36.5, bleeding: Bleeding.spotting));
    }));
    ...
    final spottingDots = find.descendant(
        of: _tileMarker(_spottingDay), matching: find.byType(BleedingFill));
    expect(spottingDots.evaluate().length, greaterThanOrEqualTo(3),
