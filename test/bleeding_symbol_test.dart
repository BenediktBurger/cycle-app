...
void main() {
  ...
  testWidgets(
      'level none renders no fill (the surfaces keep their own '
      'none appearance)', (tester) async {
    await pumpSymbol(tester, Bleeding.none);
    expect(find.byType(BleedingFill), findsNothing,
        reason: 'an untracked-looking empty box, nothing painted');
  });

  testWidgets(
      'levels 2–5 fill (level-1)/4 of the box height, '
    ...
    for (final MapEntry(key: level, value: fraction) in expectations.entries) {
      ...
      expect(rect.height, closeTo(boxSide * fraction, 0.01),
          reason: '$level fills $fraction of the box height');
      expect(rect.bottom, closeTo(box.bottom, 0.01),
          reason: '$level: the fill is anchored at the box\'s bottom');
      expect(rect.width, closeTo(boxSide, 0.01),
  ...
  testWidgets('the fill carries the theme error color', (tester) async {
    ...
    expect(tester.widget<BleedingFill>(find.byType(BleedingFill)).color,
        scheme.colorScheme.error);
  });

  testWidgets(
      'spotting renders dots — three or more disjoint fill regions '
    ...
    expect(sorted.last.right - sorted.first.left, lessThan(boxSide),
